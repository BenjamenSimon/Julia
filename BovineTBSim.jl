...
function create_farm_res(parish, farm, Time, cS, cI, bS, bI)
  ...
  farm_res = DataFrame(fill(Float64, 10),
...
function create_farm_track(Time)
  ...
  farm_track = DataFrame(fill(Float64, 27),
                     [:t,
...
function create_parish_res(parish, Time, farms)
  ...
  # Inputs
  inputs = DataFrame(fill(Float64, 7), [:parish, :farm, :Time, :cS, :cI, :bS, :bI], farms)
  ...
  parish_res = DataFrame(fill(DataFrame, (farms+1)), fill(:farm, (farms+1)),
                         1, makeunique = true)
  select!(parish_res, Not(1)) #nice names
...
function create_parish_track(Time, farms)
  ...
  parish_track = DataFrame(fill(DataFrame, (farms+1)), fill(:farm, (farms+1)),
                           1, makeunique = true)
  select!(parish_track, Not(1)) # nice names
...
function create_country_res(Time, parishes)
  ...
  # Inputs
  inputs = DataFrame(fill(Float64, 3), [:parish, :Time, :farms], parishes)
  ...
  # Country
  country_res = DataFrame(fill(DataFrame, (parishes+1)), fill(:parish, (parishes+1)),
                          1, makeunique = true)
  select!(country_res, Not(1)) # nice names

  for i = 1:parishes
    country_res[1, i] = create_parish_res(inputs[i, 1], inputs[i, 2], inputs[i, 3])
  end
  return(country_res)
end
...
function create_country_track(Time, parishes, farms)
  ...
  # inputs
    inputs = DataFrame(fill(Float64, 2), [:Time, :farms], parishes)
    ...
    # Country
    country_track = DataFrame(fill(DataFrame, (parishes+1)), fill(:parish, (parishes+1)),
                            1, makeunique = true)
    select!(country_track, Not(1)) # nice names

    for i = 1:parishes
      country_track[1, i] = create_parish_track(inputs[i, 1], inputs[i, 2])
    end
...
function generate_moves(t, current_states)
  ...
  movements[:, :move_res] .= current_states[:, :move_res]

  move_free = findall(movements[:, :move_res] .== 0)
...
function update_states(t, parish, farm, p_env, movement)
  ...
  test_date = farm_i_track[t, :test_date]
  move_res = farm_i_res[t, :move_res]

  #########################################
  ### Calculate environmental reservoir ###
  #########################################

  env = farm_i_track[t, :env]
  ...
  sus_move_off = (movement[move_row, :cS_move_off])[1]
  exp_move_off = (movement[move_row, :cE_move_off])[1]
  inf_move_off = (movement[move_row, :cI_move_off])[1]

  sus_move_on = (movement[move_row, :cS_move_on])[1]
  exp_move_on = (movement[move_row, :cE_move_on])[1]
  inf_move_on = (movement[move_row, :cI_move_on])[1]
  ...
  new_state[:cS] = c_S_new
  new_state[:cE] = c_E_new
  new_state[:cI] = c_I_new

  new_state[:bS] = b_S_new
  new_state[:bE] = b_E_new
  new_state[:bI] = b_I_new
  new_state[:move_res] = move_res
